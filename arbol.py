...
def construir_arbol_desde_tokens(resultado):
    """
    Convierte el resultado de AnalizadorLexico.analizar() en un NodoArbol.

    Jerarquía generada:
      Programa
        Bloque (PALABRA_RESERVADA seguida de '{')
          Nombre del bloque
          Sentencia
            operador
              valor
    ...
    Si el código no tiene bloques con llaves, todos los tokens
    cuelgan directamente de Programa agrupados por ';'.
    """
    ...
    # seguridad: si nada se añadió, colgar todos los lexemas directo
    if not raiz.hijos:
...
# ── parseo con bloques  RESERVADA NOMBRE { ... } ────────────────────
def _parsear_con_bloques(tokens, raiz):
    ...
    while i < n:
        ...
        if tok["token"] == "PALABRA_RESERVADA":
            # comprobar si hay una '{' hacia adelante (antes del próximo '}')
            j = i + 1
            ...
            if hay_bloque:
                ...
                # tokens entre la palabra reservada y la '{' → hijos directos
                ...
                # palabra reservada sin bloque → hoja directa
                raiz.agregar_hijo(NodoArbol(tok["lexema"]))
                i += 1
        else:
            i += 1
...
# ── parseo plano (sin llaves), agrupado por ';' ──────────────────────
...
# ── construir sub-árbol de una sentencia ─────────────────────────────
def _agregar_grupo(padre, grupo):
    """
    Dada una lista de tokens de una sentencia, los encadena en árbol:
      [HP, =, 100]  →  HP → = → 100
    El primer token es hijo de padre, cada siguiente es hijo del anterior.
    """
    if not grupo:
        return
...
# ─────────────────────────────────────────────
#  MOTOR DE LAYOUT (Reingold-Tilford simplificado)
# ─────────────────────────────────────────────
RADIO      = 24
H_GAP      = 22
V_GAP      = 72
...
def _dibujar_nodos(canvas, nodo, fuente_rama, fuente_hoja, es_raiz=False):
    if nodo.es_hoja:
        ...
        canvas.create_text(nodo._x, nodo._y, text=nodo.etiqueta,
                           fill=texto, font=fuente_rama, anchor="center")

    for hijo in nodo.hijos:
        _dibujar_nodos(canvas, hijo, fuente_rama, fuente_hoja, es_raiz=False)


def dibujar_arbol(canvas, raiz):
    canvas.delete("all")
    fuente_rama = font.Font(family="Segoe UI", size=9, weight="bold")
    fuente_hoja = font.Font(family="Consolas",  size=10)
...
# ─────────────────────────────────────────────
#  PESTAÑA LISTA PARA INSERTAR EN EL NOTEBOOK
# ─────────────────────────────────────────────
class PestanaArbol:
    def __init__(self, parent):
        ...
        top.pack(fill="x")
        tk.Label(top, text="ÁRBOL SINTÁCTICO",
                 bg="#1e293b", fg="#38bdf8",
                 font=("Segoe UI", 9, "bold")).pack(side="left", padx=14, pady=8)
        self._lbl_info = tk.Label(top, text="", bg="#1e293b", fg="#64748b",
                                  font=("Segoe UI", 9))
        self._lbl_info.pack(side="right", padx=14)

        leyenda = tk.Frame(self.frame, bg="#0f172a")
        leyenda.pack(fill="x", padx=14, pady=(6, 0))
        for sym, color, label in [("●", "#378ADD", "Raíz"),
                                   ("●", "#38bdf8", "Rama"),
                                   ("—", "#38bdf8", "Hoja")]:
            tk.Label(leyenda, text=sym, fg=color, bg="#0f172a",
                     font=("Segoe UI", 11)).pack(side="left")
            tk.Label(leyenda, text=f" {label}   ", fg="#94a3b8", bg="#0f172a",
                     font=("Segoe UI", 9)).pack(side="left")

        wrap = tk.Frame(self.frame, bg="#020617")
        wrap.pack(fill="both", expand=True, padx=10, pady=10)

        self.canvas = tk.Canvas(wrap, bg="#020617", highlightthickness=0,
        ...
        self.canvas.create_text(300, 200,
                                text="Ejecuta el análisis para ver el árbol",
                                fill="#334155", font=("Segoe UI", 13))

    def mostrar(self, raiz):
        dibujar_arbol(self.canvas, raiz)
        n = self._contar(raiz)
